...
INDIA_RSS: List[tuple] = [
    ...
    ("Multibagger stocks (Google)",
     ["https://news.google.com/rss/search?q=india+multibagger+smallcap+stock+NSE+2026&hl=en-IN&gl=IN&ceid=IN:en"],
     "Markets"),
...
WORLD_RSS: List[tuple] = [
    ...
    ("China Economy (Google)",
     ...
     "Global Economy"),
    ("Dollar Rupee (Google)",
     ["https://news.google.com/rss/search?q=dollar+rupee+exchange+rate+forex+india&hl=en-IN&gl=IN&ceid=IN:en"],
     "Forex"),
]
...
# NSE-specific fallbacks (completely different from BSE fallbacks)
NSE_RSS_FALLBACKS = [
    # SEBI/NSE governance and compliance filings
    "https://news.google.com/rss/search?q=%22NSE%22+%22SEBI%22+%22insider+trading%22+OR+%22shareholding+pattern%22+OR+%22corporate+governance%22+india+2026&hl=en-IN&gl=IN&ceid=IN:en",
    # NSE quarterly results and earnings
    "https://news.google.com/rss/search?q=%22NSE%22+%22Q4+FY26%22+OR+%22quarterly+results%22+OR+%22earnings%22+india+stock+2026&hl=en-IN&gl=IN&ceid=IN:en",
    # Business Standard companies (distinct from ET Markets used for BSE)
    "https://www.business-standard.com/rss/companies-1021.rss",
    # NSE IPO and new listings
    "https://news.google.com/rss/search?q=%22NSE%22+IPO+listing+%22allotment%22+OR+%22subscription%22+india+2026&hl=en-IN&gl=IN&ceid=IN:en",
    # CNBC TV18 market alerts
    "https://www.cnbctv18.com/commonfeeds/v1/eng/rss/market.xml",
]
NSE_GOOGLE_URL = NSE_RSS_FALLBACKS[1]
...
def fetch_nse() -> List[Dict]:
    ...
    # Fallback: Try multiple RSS sources for NSE corporate news
    ...
    for rss_url in NSE_RSS_FALLBACKS:
        ...
        if len(items) >= 50:
            print(f"    ✓ NSE RSS ({rss_url.split('/')[2][:30]}): {len(items)} items")
            break

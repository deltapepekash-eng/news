...
def fetch_search_feed(terms: List[str], india_items: List[Dict], world_items: List[Dict]) -> List[Dict]:
    """
    Build Search Feed: items from all RSS sources that match ANY search term.
    Returns up to MAX_ITEMS items tagged with matched_terms.
    """
    all_items = india_items + world_items
    # An item qualifies if at least one term matches title or summary
    qualified = []
    seen_ids = set()
    for item in all_items:
        if item["id"] in seen_ids:
            continue
        mt = item.get("matched_terms", [])
        if mt:  # already tagged by fetcher
            qualified.append(item)
            seen_ids.add(item["id"])
    # Sort by recency
    try:
        qualified.sort(key=lambda x: x.get("published_at", ""), reverse=True)
    except Exception:
        pass
...
def extract_split_trending(india_items: List[Dict], world_items: List[Dict]) -> tuple:
    """
    Extract top 20 India trending and top 20 World trending separately.
    Returns (india_trends, world_trends)
    """
    def count_trends(items):
        counter: Counter = Counter()
        for item in items:
            text = (item.get("title","") + " " + item.get("summary","")).lower()
            if any(b in text for b in BLOCKLIST):
    ...
    return count_trends(india_items), count_trends(world_items)
...
def main_v22():
    ...
    print(f"  India top: {', '.join(t['term'] for t in india_trends[:4])}")
    print(f"  World top: {', '.join(t['term'] for t in world_trends[:4])}\n")

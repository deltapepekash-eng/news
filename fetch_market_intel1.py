...
def fetch_bse() -> List[Dict]:
    ...
    # BSE corporates API — all major announcement categories
    # subcategory=-1 means all, category codes: -1=all, 1=corp action, 2=fin results, etc.
    BSE_CAT_URLS = [
        # Corporate Announcements (all)
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=-1&strPrevDate={prev}&strScrip=&strSearch=P"
        ...
        try:
            ...
            for row in table[:150]:
                ...
                if not company or company.isupper() and len(company.split()) >= 3:
                    # Skip pure personal names (all-caps multi-word)
                    company = (row.get("SCRIP_CD") or "Unknown")
                ...
                link     = (f"https://www.bseindia.com/xml-data/corpfiling/AttachHis/{att}"
                            if att else f"https://www.bseindia.com/corporates/ann.html?scrip={row.get('SCRIP_CD','')}")
                ts = now_iso()
    ...
    # Last resort: Google News (market-relevant only, no personal names)
...
def main_v22():
    ...
    if fresh_bse:
        ...
        bse_items = merge(clean_old_bse, fresh_bse, cap=300)
    else:
    ...
    if fresh_nse:
        ...
        nse_items = merge(clean_old_nse, fresh_nse, cap=300)
    else:

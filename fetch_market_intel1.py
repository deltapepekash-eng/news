...
BSE_RSS_URLS = [
    # BSE official RSS — all corporate announcements categories
...
def fetch_bse() -> List[Dict]:
    ...
    # ── Try 1: All BSE RSS feeds (official + feeder.co mirror) ────
    for rss_url in BSE_RSS_URLS:
        raw = fetch_url(rss_url, {"Referer": "https://www.bseindia.com/",
        ...
        try:
            ...
            if batch:
                items.extend(batch)
                print(f"    ✓ BSE RSS ({rss_url.split('/')[-1][:30]}): {len(batch)} items")
        except Exception as e:
            print(f"    ✗ BSE RSS {rss_url[:50]}: {e}")
    ...
    # ── Try 2: BSE JSON API (all 5 announcement categories) ───────
    BSE_CAT_URLS = [
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=-1&strPrevDate={prev}&strScrip=&strSearch=P"
         "&strToDate={today}&strType=C&subcategory=-1", "Announcement"),
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=9&strPrevDate={prev}&strScrip=&strSearch=P"
         "&strToDate={today}&strType=C&subcategory=-1", "Corporate Action"),
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=6&strPrevDate={prev}&strScrip=&strSearch=P"
         "&strToDate={today}&strType=C&subcategory=-1", "Financial Results"),
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=7&strPrevDate={prev}&strScrip=&strSearch=P"
         "&strToDate={today}&strType=C&subcategory=-1", "Board Meeting"),
        ("https://api.bseindia.com/BseIndiaAPI/api/AnnSubCategoryGetData/w"
         "?strCat=5&strPrevDate={prev}&strScrip=&strSearch=P"
         "&strToDate={today}&strType=C&subcategory=-1", "Insider Trading"),
    ]
    today    = datetime.now(IST).strftime("%Y%m%d")
    week_ago = (datetime.now(IST) - timedelta(days=14)).strftime("%Y%m%d")

    for url_tmpl, cat_label in BSE_CAT_URLS:
        url = url_tmpl.format(prev=week_ago, today=today)
        ...
        try:
            ...
            for row in table[:150]:
                ...
                if not company or (company.isupper() and len(company.split()) >= 3):
                    company = str(row.get("SCRIP_CD","Unknown"))
                ann_raw  = row.get("CATEGORYNAME") or row.get("SUBCATEGORYNAME") or cat_label
                ...
                link     = (f"https://www.bseindia.com/xml-data/corpfiling/AttachHis/{att}"
                            if att else f"https://www.bseindia.com/corporates/ann.html")
                ts = now_iso()
                ...
                items.append({
                    ...
                    "detail": headline, "url": link, "published_at": ts, "exchange": "BSE",
                })
        except Exception as e:
    ...
    if items:
        items.sort(key=lambda x: x.get("published_at",""), reverse=True)
        print(f"    ✓ BSE API total: {len(items)} items")
        return items[:300]

    # ── Try 3: BSE-specific fallback URLs (different from NSE) ────
    print("    → BSE fallback URLs")
    for fallback_url in BSE_FALLBACK_URLS:
        ...
        seen_ids.add(uid)
        items.append({
            "id": uid, "company": title[:100], "announcement_type": "Announcement",
...
def fetch_nse() -> List[Dict]:
    ...
    week_ago = (datetime.now(IST) - timedelta(days=14)).strftime("%d-%m-%Y")
    ...
    if raw:
        try:
            ...
            for row in records[:MAX_ITEMS]:
                # Prefer sm_name (full company name) over symbol (ticker)
                company  = clean_html_entities(
                ...
                items.append({
                    "id": make_id(company + dt_str + ann_type),
                    "company": company,
                    "announcement_type": ann_type,
                    "detail": detail,
                    "url": url_link or "https://www.nseindia.com/companies-listing/corporate-filings-announcements",
                    "published_at": ts,
                    "exchange": "NSE",
                })
            if items:
    ...
    # ── Try 2: Official NSE RSS feeds (nsearchives.nseindia.com) ──
    print("    → NSE official RSS feeds")
    seen_ids: set = set()
        ...
        try:
            ...
            for e in root.findall(".//item"):
                ...
                if uid in seen_ids:
                    continue
                seen_ids.add(uid)
                # Extract company from title (format: "COMPANY - Announcement Type")
                parts = title.split(" - ", 1)
                company  = parts[0][:100].strip()
                ann_type = parts[1][:80].strip() if len(parts) > 1 else "Announcement"
                batch.append({
            ...
            print(f"      ✗ {rss_url.split('/')[-1]}: {e}")
            continue
        if batch:
            items.extend(batch)
            print(f"      ✓ {rss_url.split('/')[-1]}: {len(batch)} items")

    if items:
        items.sort(key=lambda x: x.get("published_at",""), reverse=True)
        print(f"    ✓ NSE official RSS total: {len(items)} items")
        return items[:500]

    # ── Try 3: Fallback RSS sources ────────────────────────────────
    print("    → NSE RSS fallbacks")
    for rss_url in NSE_RSS_FALLBACKS:
        batch = []
        for item in fetch_rss("NSE RSS", [rss_url], "Corporate", []):
            title = clean_bse_title(item["title"])
        ...
        if batch:
            items.extend(batch)
            print(f"    ✓ NSE {rss_url.split('/')[2][:25]}: {len(batch)} items")

    items.sort(key=lambda x: x.get("published_at",""), reverse=True)
    print(f"    ✓ NSE fallback total: {len(items)} items")
    return items[:300]
